...
void CGyfxyDebugView::OnUpdate(CView* /*pSender*/, LPARAM /*lHint*/, CObject* /*pHint*/)
{
	...
	GetDlgItem(IDC_BUTTON_LEFT_WARM)->SetWindowText(pFuzzy[LEFT_STOVE].WarmFlag?_T("Í£Ö¹"):_T("ÉýÎÂ"));
	GetDlgItem(IDC_BUTTON_RIGHT_WARM)->SetWindowText(pFuzzy[RIGHT_STOVE].WarmFlag?_T("Í£Ö¹"):_T("ÉýÎÂ"));
	GetDlgItem(IDC_BUTTON_WATER_STOVE_WARM)->SetWindowText(pFuzzy[WATER_STOVE].WarmFlag?_T("Í£Ö¹"):_T("ÉýÎÂ"));
	...
	GetDlgItem(IDC_BUTTON_SAMPLE_ROUND)->SetWindowText(pGyfxyRdb->status.bSampleDiskMoving?_T("Í£Ö¹"):_T("Ðý×ª"));
	GetDlgItem(IDC_BUTTON_SAMPLENO_MOVE)->SetWindowText(pGyfxyRdb->status.bSampleDiskMoving?_T("Í£Ö¹"):_T("ÒÆ¶¯"));
	...
	GetDlgItem(IDC_BUTTON_LEFT_OXYGEN)->SetWindowText(pOutport->LEFT_OXYGEN?_T("ÑõÆø¹Ø"):_T("ÑõÆø¿ª"));
	GetDlgItem(IDC_BUTTON_RIGHT_OXYGEN)->SetWindowText(pOutport->RIGHT_NITROGEN?_T("µªÆø¹Ø"):_T("µªÆø¿ª"));

	if(szSystemType==_T("G5200"))
	{
		GetDlgItem(IDC_BUTTON_LEFT_FAN)->SetWindowText(pOutport->LEFT_STOVE_FAN_COVER?_T("×óÂ¯·çÉÈ¹Ø"):_T("×óÂ¯·çÉÈ¿ª"));
		GetDlgItem(IDC_BUTTON_RIGHT_FAN)->SetWindowText(pOutport->RIGHT_STOVE__DRAFT_FAN?_T("ÓÒÂ¯·çÉÈ¹Ø"):_T("ÓÒÂ¯·çÉÈ¿ª"));
	}
	else if(szSystemType==_T("G5000"))
	{
		GetDlgItem(IDC_BUTTON_LEFT_FAN)->SetWindowText(pOutport->LEFT_STOVE_FAN_COVER?_T("¹ØÂ¯¸Ç"):_T("¿ªÂ¯¸Ç"));
		GetDlgItem(IDC_BUTTON_RIGHT_FAN)->SetWindowText(pOutport->RIGHT_STOVE__DRAFT_FAN?_T("ÅÅ·çÉÈ¹Ø"):_T("ÅÅ·çÉÈ¿ª"));
	}
	GetDlgItem(IDC_BUTTON_WATER_STOVE)->SetWindowText(pOutport->WATER_STOVE_FAN?_T("ºãÎÂÂ¯·çÉÈ¹Ø"):_T("ºãÎÂÂ¯·çÉÈ¿ª"));

 	GetDlgItem(IDC_BUTTON_LEFT_UP)->SetWindowText(pOutport->LEFT_MOTOR_UP?_T("Í£Ö¹"):_T("ÉÏÉý"));
	GetDlgItem(IDC_BUTTON_LEFT_DN)->SetWindowText(pOutport->LEFT_MOTOR_DN?_T("Í£Ö¹"):_T("ÏÂ½µ"));

	GetDlgItem(IDC_BUTTON_RIGHT_UP)->SetWindowText(pOutport->RIGHT_MOTOR_UP?_T("Í£Ö¹"):_T("ÉÏÉý"));
	GetDlgItem(IDC_BUTTON_RIGHT_DN)->SetWindowText(pOutport->RIGHT_MOTOR_DN?_T("Í£Ö¹"):_T("ÏÂ½µ"));

	GetDlgItem(IDC_BUTTON_WATER_STOVE_UP)->SetWindowText(pOutport->BALANCE_MOTOR_UP?_T("Í£Ö¹"):_T("ÉÏÉý"));
	GetDlgItem(IDC_BUTTON_WATER_STOVE_DN)->SetWindowText(pOutport->BALANCE_MOTOR_DN?_T("Í£Ö¹"):_T("ÏÂ½µ"));
...
void CGyfxyDebugView::OnBnClickedButtonLeftOxygen()
{
	...
	pHostCtrl->AddCommand(COMMAND_JDQ,COIL_LEFT_OXYGEN,pOutport->LEFT_OXYGEN?0:1);
		...
		#define  COIL_BALANCE_MOTOR_UP	0;		//³ÆÑùÉÏÉý
		#define  COIL_BALANCE_MOTOR_DN	1;		//³ÆÑùÏÂ½µ
		#define  COIL_LEFT_MOTOR_UP		2;		//×óÂ¯ÉÏÉý
		#define  COIL_LEFT_MOTOR_DN		3;		//×óÂ¯ÏÂ½µ

		#define  COIL_RIGHT_MOTOR_UP	4;		//ÓÒÂ¯ÉÏÉý
		#define  COIL_RIGHT_MOTOR_DN	5;		//ÓÒÂ¯ÏÂ½µ	

		#define  COIL_WATER_STOVE_FAN	:8;		//Ë®·ÖÂ¯×Ó½»Á÷·çÉÈ 
		#define  COIL_LEFT_STOVE_FAN	:11;		//×óÂ¯·çÉÈ

		#define  COIL_RIGHT_STOVE_FAN	:12;		//ÓÒÂ¯·çÉÈ
		#define  COIL_SPK				:13;		//·äÃùÆ÷*/
}
...
{ 
	...
	//cCommandParam1;    //µç»ú±àºÅ
	//cCommandParam2;	 //ÊÇ·ñÔÊÐí
	//cCommandParam3;    //µç»úÔËÐÐÄ¿±ê
