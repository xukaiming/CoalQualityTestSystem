...
XLISTCTRLLIBDLLEXPORT UINT WM_XLISTCTRL_COMBO_SELECTION			= ::RegisterWindowMessage(_T("WM_XLISTCTRL_COMBO_SELECTION"));
														                            //				
XLISTCTRLLIBDLLEXPORT UINT WM_XLISTCTRL_DATETIMECTRL_SELECTION  = ::RegisterWindowMessage(_T("WM_XLISTCTRL_DATETIMECTRL_SELECTION"));
...
#endif
#ifndef DO_NOT_INCLUDE_XDATETIMEPICKER
	ON_WM_TIMER()
	ON_REGISTERED_MESSAGE(WM_XCOMBOLIST_VK_ESCAPE, OnDateTimeCtrlEscape)
	ON_REGISTERED_MESSAGE(WM_XCOMBOLIST_COMPLETE, OnDateTimeCtrlComplete)
#endif
...
#ifndef DO_NOT_INCLUDE_XDATETIMEPICKER	
	//-----------------------------------------
	m_bDateTimePickerIsClicked	= FALSE;
	m_nDateTimePickerItem		= 0;
	m_nDateTimePickerSubItem	= 0; 
	m_pDateTimePicker			= NULL;
	m_bDateTimeFontIsCreated	= FALSE;
#endif	
	//---------------------------------------------------------------------
...
void CXListCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	else
	{
		...
		else
		{
			...
			else if (pXLCD[nSubItem].bDateTimeCtrl)
			{
				CListCtrl::OnLButtonDown(nFlags, point);
				DrawDateTimePicker(nItem, nSubItem);
			}
...
#ifndef DO_NOT_INCLUDE_XDATETIMEPICKER
LRESULT CXListCtrl::OnDateTimeCtrlEscape(WPARAM, LPARAM)
{
	XLISTCTRL_TRACE(_T("in CXListCtrl::OnDateTimeCtrlEscape\n"));
	SetTimer(2, 50, NULL);
	//UpdateSubItem(m_nComboItem, m_nComboSubItem);
	return 0;
}

LRESULT CXListCtrl::OnDateTimeCtrlComplete(WPARAM, LPARAM)
{
	XLISTCTRL_TRACE(_T("in CXListCtrl::OnDateTimeCtrlComplete\n"));
	SetTimer(3, 50, NULL);
	return 0;
}
#endif
...
void CXListCtrl::OnTimer(UINT nIDEvent)
{
	...
	case 2:
		...
		if (m_pDateTimePicker)
		{
			UpdateSubItem(m_nDateTimePickerItem, m_nDateTimePickerSubItem);
			m_pDateTimePicker->DestroyWindow();
			delete m_pDateTimePicker;
		}
		m_pDateTimePicker = NULL;
		break;
	case 3:
		...
		if (m_pDateTimePicker)
		{
			...
			if (!pWnd)
				pWnd = GetOwner();
			if (pWnd && ::IsWindow(pWnd->m_hWnd))
				pWnd->SendMessage(WM_XLISTCTRL_DATETIMECTRL_SELECTION, 
				m_nDateTimePickerItem, m_nDateTimePickerSubItem);
			/*
			CString str;
			int i = m_pCombo->GetCurSel();
			if (i != LB_ERR)
			{
					...
					(m_nComboSubItem >= 0 && m_nComboSubItem < GetColumns()))
				{
					...
					UpdateSubtem(m_nComboItem, m_nComboSubItem);
...
#ifndef DO_NOT_INCLUDE_XDATETIMEPICKER   //DO_NOT_INCLUDE_XCOMBOLIST
//void	CXListCtrl::DrawDateTimePicker(int nItem, 
//						   int nSubItem)

void	CXListCtrl::DrawDateTimePicker(int nItem,int nSubItem)
{
	ASSERT(nItem >= 0);
	ASSERT(nItem < GetItemCount());
	if ((nItem < 0) || nItem >= GetItemCount())
	...
	if ((dwExStyle & LVS_EX_FULLROWSELECT) == 0)
	{
		XLISTCTRL_TRACE(_T("DrawDateTimePicker: combo boxes require LVS_EX_FULLROWSELECT style\n"));
		ASSERT(FALSE);
	}
#endif
	...
	m_pDateTimePicker = new CXDateTimeCtrl();
	ASSERT(m_pDateTimePicker);
	//Create(WS_TABSTOP | WS_CHILD | WS_VISIBLE |/* WS_BORDER |*/ DTS_SHORTDATEFORMAT,
	//	CRect(0, 0, 100, 22), this, 0);
	if (m_pDateTimePicker)
	{
		m_nDateTimePickerItem		= nItem;
		m_nDateTimePickerSubItem	= nSubItem;

		rect.top -= 1;
		m_rectDateTimePickerButton = rect;
		ClientToScreen(&m_rectDateTimePickerButton);
		m_rectDateTimePickerButton.left += 1;

		DWORD dwStyle = CBS_DROPDOWNLIST | WS_POPUP | WS_VISIBLE ;
		/*
		BOOL bSuccess = m_pDateTimePicker->CreateEx(WS_EX_CONTROLPARENT, 
			ADVCOMBOBOXCTRL_CLASSNAME, 
			_T(""),
			dwStyle,
			m_rectComboList,
			...
			NULL);
		*/
		BOOL bSuccess = m_pDateTimePicker->Create(WS_TABSTOP | WS_CHILD | WS_VISIBLE |
			/* WS_BORDER |*/ DTS_SHORTDATEFORMAT,
				m_rectDateTimePickerButton, this, 0);
		COleDateTime dt;  
		if (bSuccess)
		{
			LockWindowUpdate();
			...
			try
			{
				dt = pXLCD[nSubItem].m_dt;
				if (dt.GetStatus()!=COleDateTime::valid)
				{
					dt = COleDateTime::GetCurrentTime();							
				} 
				m_pDateTimePicker->SetTime(dt);				
			}
			catch(...)
			{
				TRACE(_T("ERROR - exception in CXListCtrl::DrawDateTimePicker\n")); 
			}
		}
		...
		if(dt.GetStatus()!=COleDateTime::valid)
		{
			dt.ParseDateTime(GetItemText(nItem, nSubItem));	
		} 
		m_strInitialString = dt.Format(_T("%Y-%m-%d %H-%M-%S")); 
		SetTimer(1, 50, NULL);
		

		m_pDateTimePicker->Invalidate();
		m_pDateTimePicker->RedrawWindow();
		m_pDateTimePicker->BringWindowToTop();
	}
}
#endif
...
BOOL CXListCtrl::SetDatetimeCtrl(int nItem, 
								 int nSubItem, 
								 BOOL bEnableDateTimeCtrl, 
								 int nDateTimeCtrlHeight, 
								 COleDateTime dt)
{
	ASSERT(nItem >= 0);
	ASSERT(nItem < GetItemCount());
	...
	// update flag
	pXLCD[nSubItem].bDateTimeCtrl = bEnableDateTimeCtrl;

	if (bEnableDateTimeCtrl)
	{
		// sort CStringArray before setting initial selection 
 
		pXLCD[nSubItem].nDateTimeCtrlHeight = nDateTimeCtrlHeight;
		pXLCD[nSubItem].m_dt = dt; 
